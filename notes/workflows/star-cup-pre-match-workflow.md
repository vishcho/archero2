...
> 正式網站不再從 Markdown 即時計算建議。抽取賽前對陣時，若能從畫面連線逐場確認 R1，應同步直接建立符合
...
盤點指令：`node tools/check-screenshots.mjs --round roundN`（`matchup` 與 `rank` 兩欄都要是 ✅）。
目錄命名與四類批次的規範見 [`screenshots/README.md`](../../screenshots/README.md)。

同輪的 `top64` 批次（64 張選手個人資訊名片）**不是本流程的輸入**，走
[選手檔案工作流](./top64-profile-workflow.md)。但它能用用戶 ID 確認本流程只能標
`⚠ 同名多筆` / `≈ 疑同一人` 的對應——若該批已分析完，本流程的同名處理可直接引用其結論。
...
- **本期主題**只出現在此畫面頂部（對陣圖上沒有），是 `theme` 欄位唯一來源。畫面原句形如
  「獲得精靈和飛劍流派技能」，寫入 `theme` 前要簡化成 `流派Ax流派B`（本例為 `精靈x飛劍`）：
  只取兩個流派名、以半形小寫 `x` 相接，去掉「獲得／和／流派技能」且不加空白，流派名照原字。
...
上述限制適用於正式賽季 JSON。若賽前畫面本身能沿連線確認四場 R1，另建下注預覽專用的
`tmp/<season>-matchup.json`，直接使用 `schemas/matchup.schema.json` 的 `groups[].matches[]`；
它不含 power 或賽果，也不併入賽季 JSON。看不清連線時不得用 `players[]` 索引補配，應停止預覽並列出待人工確認的場次。
...
若同批另產生下注預覽 matchup，再獨立檢查：每組剛好四場 A、B、C、D；八位玩家各出現一次；
`name + player_id` 與本屆賽季名單一致；並執行 `npm run predictions:preview -- <season> --matchup tmp/<season>-matchup.json`。
...
執行語意（2026-07-17 依使用者確認）：高順位指標分得出勝負就定案；低順位指標**只在高順位打平或缺資料時遞補**，不得推翻高順位的明顯差距。低順位指標強烈反向時不改押注，只降一級信心並寫入風險提醒。
