...
const names = ["cups", "season", "roster", "players", "prediction", "matchup"];
...
for (const name of names)
  validators.set(
    name,
    ajv.compile(await readJson(schemaPath(`${name}.schema.json`))),
  );
...
export function assertSchema(name, value, file) {
  ...
  if (errors.length)
    throw new Error(
      errors.map((e) => `${e.file}${e.location}: ${e.message}`).join("\n"),
    );
}
