...
const {
  ...
  matchSideToPlayer,
  bracketAdvancementIssues,
  playerSlotMap,
} = await loadBrowserScript("js/domain.js");
...
const eightPlayers = ["A", "B", "C", "D", "E", "F", "G", "H"].map(
  (name, i) => ({
    name,
...
test("bracket 結構完全由 matches 的 round/slot 決定，與 players 順序無關", () => {
  ...
  const shuffled = {
    ...cleanGroup,
    players: [...cleanGroup.players].reverse(),
  };
...
test("matches 順序被打亂時仍依 slot 正確排列", () => {
  const scrambled = {
    ...cleanGroup,
    matches: [...cleanGroup.matches].reverse(),
  };
...
test("player_id 存在時優先以 id 關聯，不受同名影響", () => {
  const players = [
    { name: "同名", player_id: "1", prev_best: "1強" },
    { name: "同名", player_id: "2", prev_best: "64強" },
  ];
...
test("side 有 player_id 但找不到時不得退回名稱關聯", () => {
  ...
  const result = matchSideToPlayer(
    { name: "牛大力", player_id: "102045250" },
    players,
  );
...
test("雙方同名時勝者身分標記為 unverifiable（共識規則 7d）", () => {
  const players = [
    { name: "牛大力", player_id: "1" },
    { name: "牛大力", player_id: "2" },
  ];
  const group = {
    id: 1,
    players,
    matches: [
      {
        round: "R2",
        slot: "upper",
        p1: side("牛大力", { power: "42.72M" }),
        p2: side("牛大力", { power: "30.53M" }),
        winner: "牛大力",
        loser: "牛大力",
      },
    ],
...
test("缺少 slot 與重複 slot 會被記錄而非丟失", () => {
  const partial = {
    id: 1,
    players: [],
    matches: [match("R1", "A", "A", "B", "A")],
  };
  const model = buildBracketViewModel(partial);
  assert.equal(model.r1.length, 4, "缺少的 slot 仍須佔位，避免畫面錯位");
  assert.ok(
    model.diagnostics.filter((d) => d.kind === "missing-slot").length >= 4,
  );

  const duplicated = {
    id: 1,
    players: [],
    matches: [match("R1", "A", "A", "B", "A"), match("R1", "A", "X", "Y", "X")],
  };
...
test("晉級路徑檢查可偵測 R1 勝者未進入任何 R2", () => {
  assert.deepEqual(
    bracketAdvancementIssues(buildBracketViewModel(groupAB)),
    [],
  );
  assert.deepEqual(
    bracketAdvancementIssues(buildBracketViewModel(groupAC)),
    [],
    "A/C 路徑是合法資料，不得被誤報",
  );
  ...
  assert.ok(
    issues.some((i) => i.name === "A"),
    "A 未晉級任何 R2",
  );
  assert.ok(
    issues.some((i) => i.name === "B"),
    "B 不是任何 R1 勝者",
  );
});
...
test("R2 晉級來源逐組反推：A/B → upper", () => {
  ...
  assert.deepEqual([model.layout.leftTop, model.layout.rightTop].sort(), [
    "A",
    "B",
  ]);
  assert.deepEqual([model.layout.leftBottom, model.layout.rightBottom].sort(), [
    "C",
    "D",
  ]);
});
...
test("R2 晉級來源逐組反推：A/C → upper", () => {
  ...
  assert.deepEqual(
    [model.layout.leftTop, model.layout.rightTop].sort(),
    ["A", "C"],
...
test("缺 R2 資料時 layout 仍完整，不留空位", () => {
  ...
  const slots = [
    model.layout.leftTop,
    model.layout.leftBottom,
    model.layout.rightTop,
    model.layout.rightBottom,
  ];
  assert.deepEqual(
    slots.slice().sort(),
    ["A", "B", "C", "D"],
  ...
  for (const id of SEASONS) {
    ...
    for (const group of season.groups) {
      ...
      const expected = group.matches
        ...
        .sort();
      const actual = model.r1
        .map((m) => [m.slot, m.p1 && m.p1.name, m.p2 && m.p2.name])
        .sort();
      assert.deepEqual(
        actual,
        expected,
        `${id} 第 ${group.id} 組的 R1 必須完全來自 matches`,
      );
    }
  }
});
...
test("2026-07-31 第 1 組：修正前用索引規則會畫錯，view model 必須正確", async () => {
  ...
  assert.notDeepEqual(
    [model.r1[0].p1.name, model.r1[0].p2.name],
    ["牛大力", "牛大刃"],
  );
  ...
  for (const id of SEASONS) {
    ...
    for (const group of season.groups) {
      ...
      assert.deepEqual(
        actual,
        expected,
        `${id} 第 ${group.id} 組的 R2 必須完全來自 matches`,
      );
    }
  }
});
...
test("四屆真實資料：診斷分類數與稽核基準一致", async () => {
  ...
  for (const id of SEASONS) {
    ...
    for (const group of season.groups) {
      ...
      for (const m of [...model.r1, ...model.r2, model.final]) {
        for (const card of [m && m.p1, m && m.p2])
          if (card) counts[card.identity] += 1;
      }
    }
  }
  ...
  assert.equal(
    counts.ambiguous,
    5,
    "ambiguous 數量變動代表資料或關聯規則改變，需重新檢視",
  );
  assert.equal(
    counts.unmatched,
    43,
    "unmatched 數量變動代表歷史資料被修改，須有截圖佐證",
  );
