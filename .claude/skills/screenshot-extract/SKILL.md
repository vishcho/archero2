...
| 全量重抽複核 | 盲抽全批寫 `tmp/<round>-recheck/`，順序無關比對後裁決 |
...
複核開跑前先跑一次**常數欄位偵測**（見
[`references/validation-and-import.md`](references/validation-and-import.md)「常數欄位偵測」）：
「應該逐場獨立的欄位在 `data/` 中卻是常數」代表匯入時複製了單一值，是純讀 `data/`、
不花圖片 token 就能圈出可疑範圍的訊號。round4 的系統性戰力錯誤即由此發現。

重抽既有批次時，暫存檔一律放 `tmp/<round>-recheck/`（例：`tmp/round4-recheck/`），
比對腳本一併放在同目錄。這類腳本是 **report-only**：只讀 `data/`，永不寫入。
確認要改資料時走既有 importer 與 `--dry-run`，不要讓比對腳本兼任寫入工具。

## 委派 OCR 給子代理

大批圖片可委派子代理判讀以節省主對話 token。依**欄位**而非圖片切分模型：

| 欄位性質                             | 模型   | 依據                                    |
| ------------------------------------ | ------ | --------------------------------------- |
| 姓名、稱號、公會等 CJK／韓文字形     | Sonnet | 字形誤讀會安靜通過驗證，不可省          |
| 戰力、關卡、通天塔、魅力、徽記、時間 | Haiku  | 純數字實測與 Sonnet 一致，成本約 1/6    |
| 差異裁決的回看                       | Sonnet | 需要與既有值權衡，Haiku 傾向直接給答案  |

2026-08-14 於 round4 名片 16 張實測（同 prompt 盲測，ground truth 為
`data/players.json`）：`player_id` 兩者皆 16/16；累積型數字 96/96 完全一致；
但姓名 Sonnet 16/16、Haiku 10/16，其中 6 筆為**全新誤讀且未標任何 uncertain**
（`某剩啊`→`某刺啊`、`龍×똥꼬`→`龍×琦笃`、`放開隻牛仔`→`放開皇牛仨`），
另有一筆把被頭像遮住的稱號從公會名補成完整字串。Haiku 全程 0 個 `uncertain`、
0 個 null——它不是較有把握，是不會察覺自己在猜。無標記的誤讀會通過
`[UNSURE]` 與 `npm run check`，因此**姓名類欄位不得用 Haiku**。

同一張圖可拆給兩個模型讀不同欄位，兩份結果互為交叉驗證，比單一模型跑兩次更有效。

### 成本實測（2026-08-14 round4 複核，取自 transcript usage）

| 項目 | 計費 input | output | 成本 |
| --- | --- | --- | --- |
| 主對話 opus-5（210 次呼叫） | 20.66M | 312.9k | $64.42 |
| 子代理 22 個（Sonnet 16 + Haiku 5） | 25.90M | 112.8k | $16.47 |
| ├ 其中 Sonnet 16 個 | — | — | $15.20 |
| └ 其中 Haiku 5 個 | — | — | $1.27 |

單組 8 張圖的 power-only 盲讀約 **$0.5–0.9／組**，全量 8 組加補讀約 **$6.2**。

三個可據以決策的比例：

1. **主對話佔總成本約 8 成**，子代理只佔 2 成。想省錢優先減少主對話載入的圖片與
   回看次數，不是減少子代理數量。
2. **`subagent_tokens` 不是計費量**：本次名目約 1.16M，實際計費 input 25.9M，
   相差約 22 倍，差額全在圖片 cache。回報成本一律從 transcript `usage` 計算。
3. **cacheRead 是主要成本項**（主對話 20.08M／子代理 23.13M）。每多一輪對話都在
   既有 cacheRead 基數上再收一次，長 session 應適時 compact。

委派時務必在 prompt 內寫明：

- **盲測**：禁止讀 `data/`、`docs/` 或既有 `tmp/` 抽取結果。
- 讀不清即 `null` + `uncertain: true` + `uncertain_fields` + note；**猜值比留空嚴重**。
- `player_id` 為字串，不得轉 number。
- 易混字形清單（見 `references/visual-extraction.md`「委派用字形清單」）。
- 檔名順序不代表排名；同名多人是真實情況，不得去重或合併。
- 輸出檔路徑與 schema，並要求每個輸入檔對應一筆、順序一致。
- 最終訊息只回摘要，不要貼回完整 JSON（否則抵銷節省的 token）。

成本由圖片 cache 主導而非文字輸出，縮短 prompt 幾乎無效；**減少同一張圖被重複載入**
才是關鍵。裁決用的回看也應併成一批委派，不要逐張在主對話開圖。

### 子代理會幻覺配對，交叉驗證要靠獨立回讀

2026-08-14 round4 複核時，兩個 Sonnet 子代理對**同一張圖**給出互相矛盾的配對與
戰力；主對話親自開圖才確定其中一個把整場對戰的雙方張冠李戴。教訓：

- 子代理的裁決結論**不是證據**，只是待驗證的主張。最終差異一律由主對話回看原圖確認。
- 想要真正的交叉驗證，就讓兩批代理**分別盲讀同一張圖的不同欄位**（姓名／數字），
  或以**單一欄位重跑一次全量**（例如只讀 power）。同一份 prompt 跑兩次不算獨立。
- 差異裁決代理不要同時給它「找出差異」與「決定誰對」兩個任務；它會傾向編一個
  自洽的故事來同時滿足兩者。
...
3. 排行榜保留重疊列，以 rank 去重並檢查連續性。頂部「本期主題」是明星盃 `theme` 的唯一來源，
   寫入前一律簡化成 `流派Ax流派B`（原句「獲得精靈和飛劍流派技能」→ `精靈x飛劍`）：只取兩個
   流派名、以半形小寫 `x` 相接，去掉「獲得／和／流派技能」且不加空白，流派名照畫面原字。
   超級明星盃的 `theme` 是季主題（如 `精靈季 1`），照原樣記錄，不套用此簡化。
...
差異要分類到「確認錯誤／已知字形變體／累積型正常增長／排序約定差異」，不可混為一談報成錯誤總數。
若曾因排序或分類問題產生假陽性，回報時說明修正過程，不要只報最終數字。

有委派子代理時附 token 與成本明細：各代理的 output／cache write／cache read
與佔比。**注意 harness 摘要的 `subagent_tokens` 不是計費量**（round4 實測相差 23 倍，
差額來自圖片 cache），成本要從 transcript 的 usage 記錄計算。
