async function loadHome() {
  ...
  document.getElementById("app").innerHTML = `
    <section class="panel hero">
      <p class="eyebrow">最新一屆 · ${escapeHtml(state.label)}</p>
      ...
      <p class="muted" style="margin-top:18px">${published ? `正式建議發布：${escapeHtml(published)}` : "正式建議尚未發布"}${gaps.length ? `｜${escapeHtml(gaps.join("、"))}` : "｜資料完整"}</p>
    </section>
