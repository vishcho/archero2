async function loadHistory() {
  ...
  document.getElementById('app').innerHTML = `<section><p class="eyebrow">Performance</p><h1>歷屆下注成效</h1><p class="lede">只統計正式快照與可追溯的賽前文件還原；未結算場次不進入分母。</p></section><section class="grid metrics"><div class="panel metric"><span>正式累計</span><strong>${formatRate(total.correct,total.settled)}</strong></div><div class="panel metric"><span>已納入屆次</span><strong>${rows.length}</strong></div></section><section class="list">${rows.map(({ id, prediction, season, score }) => `<a class="panel list-row" href="bracket.html?id=${encodeURIComponent(id)}"><div><span class="status">${escapeHtml(prediction.source === 'snapshot' ? '正式快照' : '文件還原')}</span><h2 style="margin:8px 0 0">${escapeHtml(season.date)} 第 ${season.round} 屆</h2></div><strong>${formatRate(score.correct,score.settled)}</strong></a>`).join('')}</section>`;
}
