...
- **2026-08-09（round4）**：首批 `top64` 已分析完成，建立 `data/players.json`（63 位）。
  - 初次 64 張只涵蓋 59 位（5 位被重複點開：龍×이뮤、Yööᶠˣ、coco幻、橙色楓葉、
    秘運行者Kai）。**同日 02:17 補拍 4 張**（資格賽第 48、57、60、64 名），
    合計 68 張涵蓋 63 位。補拍證實第 64 名的「牛大办」實為 **牛大刃**（id `101821232`），
    與兩位「牛大力」是三個不同帳號。
  - 仍缺 1 位：資格賽第 46 名的「龍×똥꼬」（與第 12 名同名）。
  - **對陣表已連結 62/64 位 `player_id`**，只剩兩位「龍×똥꼬」因缺卡無法區分。
...
- 2026-08-09（同日）：首次實跑後依實作經驗修訂——
  ...
  6. 回填除了同名懸案，也把**名稱唯一者**直接掛上 `player_id`（2026-07-31 達 62/64）。
